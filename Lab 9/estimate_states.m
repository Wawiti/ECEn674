...
function xhat = estimate_states(uu, P)
   ...
   t             = uu(14);
	...
	%------------------------------------------
	%       Lateral Kalman Filter (EKF)
	%------------------------------------------
	persistent phihat;
	persistent thetahat;
	if t == 0			% Initialize variables to 0
		phihat = 0;
		thetahat = 0;
	end
	
	T_out = 0.01;						% Set Sample Rate (Much less than sample rate)
	N = 10;								% Set number of samples
	Q_attitude = 10^(-9) * diag([1 1]);	% Set Q values TUNEABLE 
	R_accel = P.sigma_accel^2;			% Set R Values TUNEABLE
	P_attitude = zeros(2,2);			% 
	for i = 1:N							% Prediction step (Solve equations)
		...
		A = [qhat*cos(phihat)*tan(thetahat)-rhat*sin(phihat)*tan(thetahat),... 
	...
	% Update the measurements (always since it will always change for lateral)
	...
	%------------------------------------------
	%    Longitudinal Kalman Filter (EKF)
	%------------------------------------------
	persistent pnhat;
    persistent pehat;
    persistent Vghat;
